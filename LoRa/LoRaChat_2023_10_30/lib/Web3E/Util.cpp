...
// TODO: changed alloca to __builtin_alloca ??? 
...
string Util::ConvertBase(int from, int to, const char *s)
{
	...
	int *fs = new (__builtin_alloca(sizeof(int)*il)) int[il];
	int k = 0;
	int i, j;
	...
	int *ts = new (__builtin_alloca(sizeof(int)*ol)) int[ol];
	int *cums = new (__builtin_alloca(sizeof(int)*ol)) int[ol];
	...
	char *out = (char*)__builtin_alloca(sizeof(char) * (ol + 1));
