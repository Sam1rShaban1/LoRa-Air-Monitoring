...
bool Crypto::Sign(uint8_t* privateKey, BYTE* digest, BYTE* result) {
...
string Crypto::GetPersonalHash(string message) {
    uint8_t challengeHash[ETHERS_KECCAK256_LENGTH];
    ...
    return Util::ConvertBytesToHex(challengeHash, ETHERS_KECCAK256_LENGTH);
}
